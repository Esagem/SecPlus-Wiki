---
title: Quiz Scaffold (TEMPLATE) — Shared Engine
category: synthesis
tags: [template, quiz, scaffold]
status: active
confidence: high
created: 2026-05-26
updated: 2026-05-26
...
# Quiz Scaffold (TEMPLATE) — Shared Engine

> **TEMPLATE FILE.** Copy this to `quizzes/_quiz-scaffold.md` in a new study wiki. The engine code is subject-independent — every quiz file in the wiki loads this file and runs its JavaScript code block via `dataviewjs`.
>
> The bootstrap stub in each individual quiz file looks like:
>
> ```dataviewjs
> const baseName = "<topic-id>-<slug>";  // matches the filename without .md
> const questions = [ /* ... */ ];
...
> ```
>
> Required caller inputs: `questions` (array of `{q, opts: [{t, c, w}, ...]}`) and `baseName` (string, no extension).
>
> Once the file is placed at `quizzes/_quiz-scaffold.md`, delete this header block.

Shared rendering and persistence engine for per-topic quizzes. Each quiz file at `quizzes/<name>.md` defines its own `questions` array and `baseName`, then loads and runs the JavaScript code block below via `dataviewjs`.
